    ...
    public static void main(String[] args) throws Exception {
        try {
            if (args.length == 0) {
                ...
                Config.fileWrite.log("Boot failed: No boot mode specified", LOG_FILE_NAME);
                System.exit(1);
            }

            String mode = args[0].toLowerCase();
            Config.fileWrite.log("Starting Loader in mode: " + mode, LOG_FILE_NAME);
            BootMode bootMode;
            ...
            switch (mode) {
                ...
                case "debug":
                    if (args.length < 2) {
                        ...
                        Config.fileWrite.log("Boot failed: Invalid debug mode syntax", LOG_FILE_NAME);
                        System.exit(1);
                    }
                ...
                default:
                    ...
                    Config.fileWrite.log("Boot failed: Invalid boot mode - " + mode, LOG_FILE_NAME);
                    System.exit(3);
                    return;
            }

            bootMode.execute();
            Config.fileWrite.log("Boot mode " + mode + " executed", LOG_FILE_NAME);
        } catch (Exception e) {
            Config.exceptionHandler.handleException(e);
            Config.fileWrite.log("Loader error: " + e.getMessage(), LOG_FILE_NAME);
...
class ProbeMode implements BootMode {
    ...
    public void execute() throws Exception {
        try {
            Config.fileWrite.log("Executing ProbeMode", Loader.LOG_FILE_NAME);
            System.exit(7);
        } catch (Exception e) {
            Config.exceptionHandler.handleException(e);
            Config.fileWrite.log("ProbeMode error: " + e.getMessage(), Loader.LOG_FILE_NAME);
...
class NormalMode implements BootMode {
    ...
    public void execute() throws Exception {
        try {
            ...
            Config.fileWrite.log("Executing NormalMode", Loader.LOG_FILE_NAME);
            new LoaderLogic().execute();
        } catch (Exception e) {
            Config.exceptionHandler.handleException(e);
            Config.fileWrite.log("NormalMode error: " + e.getMessage(), Loader.LOG_FILE_NAME);
...
class DebugMode implements BootMode {
    ...
    public void execute() throws Exception {
        try {
            Config.fileWrite.log("Executing DebugMode with option: " + debugOption, Loader.LOG_FILE_NAME);
            switch (debugOption.toLowerCase()) {
                ...
                case "astaroth":
                    ...
                    Config.fileWrite.log("DebugMode: Astaroth calendar printed", Loader.LOG_FILE_NAME);
                    System.exit(0);
                    break;
                case "io":
                    ...
                    Config.fileWrite.log("DebugMode: IO test messages printed", Loader.LOG_FILE_NAME);
                    System.exit(0);
                    break;
                default:
                    Config.io.printError("Undefined Debug Parameter.");
                    Config.fileWrite.log("DebugMode error: Undefined parameter - " + debugOption, Loader.LOG_FILE_NAME);
                    System.exit(1);
                    break;
            }
        } catch (Exception e) {
            Config.exceptionHandler.handleException(e);
            Config.fileWrite.log("DebugMode error: " + e.getMessage(), Loader.LOG_FILE_NAME);
...
class LoaderLogic {
    ...
    public void execute() throws Exception {
        try {
            Config.fileWrite.log("Starting LoaderLogic integrity checks", Loader.LOG_FILE_NAME);
            byte result = integrityChecker.checkIntegrity();

            switch (result) {
                case 0:
                    Config.io.printInfo("Integrity checks passed. Booting Cataphract...");
                    Config.fileWrite.log("Integrity checks passed, booting GuestShell", Loader.LOG_FILE_NAME);
                    new GuestShell().execute();
                    break;
                case 1:
                    Config.io.printError("Unable to locate or parse Manifest Files! Aborting boot...");
                    Config.fileWrite.log("Integrity check failed: Missing manifest files", Loader.LOG_FILE_NAME);
                    System.exit(4);
                    break;
                case 2:
                    Config.io.printError("Unable to populate the Kernel files! Aborting boot...");
                    Config.fileWrite.log("Integrity check failed: Failed to populate kernel files", Loader.LOG_FILE_NAME);
                    System.exit(4);
                    break;
                case 3:
                    Config.io.printError("File Signature verification failed! Aborting boot...");
                    Config.fileWrite.log("Integrity check failed: File signature verification", Loader.LOG_FILE_NAME);
                    System.exit(4);
                    break;
                case 4:
                    Config.io.printError("File verification failed: Found File Size Discrepancy! Aborting boot...");
                    Config.fileWrite.log("Integrity check failed: File size discrepancy", Loader.LOG_FILE_NAME);
                    System.exit(4);
                    break;
                case 5:
                    Config.fileWrite.log("Initiating Cataphract setup", Loader.LOG_FILE_NAME);
                    Setup setup = new Setup();
                    if (setup.setupCataphract()) {
                        Config.fileWrite.log("Setup completed, restarting", Loader.LOG_FILE_NAME);
                        System.exit(100);
                    } else {
                        Config.io.printError("Setup Failed!");
                        Config.fileWrite.log("Setup failed", Loader.LOG_FILE_NAME);
                        System.exit(4);
                    }
                    break;
                default:
                    Config.io.printError("Generic Failure. Cannot Boot.");
                    Config.fileWrite.log("Integrity check failed: Generic failure", Loader.LOG_FILE_NAME);
                    System.exit(4);
                    break;
            }
        } catch (Exception e) {
            Config.exceptionHandler.handleException(e);
            Config.fileWrite.log("LoaderLogic error: " + e.getMessage(), Loader.LOG_FILE_NAME);
...
class IntegrityChecker {
    ...
    public byte checkIntegrity() throws Exception {
        try {
            Config.fileWrite.log("Starting integrity checks", Loader.LOG_FILE_NAME);
            byte result = 55;

            Config.io.printInfo("Stage 0: Checking Manifest Files...");
            Config.fileWrite.log("Checking manifest files", Loader.LOG_FILE_NAME);
            if (manifestFilesCheck()) {
                Config.io.printInfo("Stage 1: Manifest Files Found. Populating Kernel Files and Directories...");
                Config.fileWrite.log("Populating kernel files", Loader.LOG_FILE_NAME);
                if (populateKernelFiles(new File("./"))) {
                    ...
                    Config.fileWrite.log("Checking file hashes", Loader.LOG_FILE_NAME);
                    if (checkFileHashes()) {
                        Config.io.printInfo("Stage 3: File Integrity Check - Phase 1 Complete. Checking File Integrity - Phase 2...");
                        Config.fileWrite.log("Checking file sizes", Loader.LOG_FILE_NAME);
                        if (checkFileSizes()) {
                            Config.io.printInfo("Stage 4: File Integrity Check - Phase 2 Complete. Checking System and User Files...");
                            Config.fileWrite.log("Checking system and user files", Loader.LOG_FILE_NAME);
                            if (!setupStatusCheck()) {
                                result = 5;
                                Config.io.printAttention("Setting up Cataphract...");
                                Config.fileWrite.log("Setup required", Loader.LOG_FILE_NAME);
                            } else {
                                result = 0;
                            }
                        } else {
            ...
            Config.fileWrite.log("Integrity check result: " + result, Loader.LOG_FILE_NAME);
            abraxisFilePathList.clear();
            System.gc();
            return result;
        } catch (Exception e) {
            Config.exceptionHandler.handleException(e);
            Config.fileWrite.log("Integrity check error: " + e.getMessage(), Loader.LOG_FILE_NAME);
            return 55;
        }
    }
    ...
    private boolean manifestFilesCheck() throws Exception {
        try {
            ...
            Config.fileWrite.log("Manifest files check: " + (exists ? "Passed" : "Failed"), Loader.LOG_FILE_NAME);
            return exists;
        } catch (Exception e) {
            Config.exceptionHandler.handleException(e);
            Config.fileWrite.log("Manifest files check error: " + e.getMessage(), Loader.LOG_FILE_NAME);
            return false;
        }
    }
    ...
    private boolean populateKernelFiles(File fileDirectory) throws Exception {
        try {
            ...
            if (fileList == null) {
                Config.fileWrite.log("Failed to list files in directory: " + fileDirectory.getPath(), Loader.LOG_FILE_NAME);
                return false;
            }
            ...
            Config.fileWrite.log("Populated " + abraxisFilePathList.size() + " kernel files", Loader.LOG_FILE_NAME);
            return true;
        } catch (Exception e) {
            Config.exceptionHandler.handleException(e);
            Config.fileWrite.log("Populate kernel files error: " + e.getMessage(), Loader.LOG_FILE_NAME);
            return false;
        }
    }
        ...
        try {
            ...
            for (String fileName : abraxisFilePathList) {
                ...
                if (manifestHash == null || !manifestHash.equals(kernelFileHash)) {
                    ...
                    Config.fileWrite.log("File hash check failed for: " + fileName, Loader.LOG_FILE_NAME);
                    return false;
                }
            }
            Config.fileWrite.log("File hash check passed", Loader.LOG_FILE_NAME);
            return true;
        } catch (Exception e) {
            Config.exceptionHandler.handleException(e);
            Config.fileWrite.log("File hash check error: " + e.getMessage(), Loader.LOG_FILE_NAME);
            return false;
        }
    }
        ...
        try {
            ...
            for (String fileName : abraxisFilePathList) {
                ...
                if (fileSize != fileSizeM2) {
                    ...
                    Config.fileWrite.log("File size check failed for: " + fileName, Loader.LOG_FILE_NAME);
                    return false;
                }
                fileCount++;
            }
            if (fileCount < manifestM2Entries.size()) {
                ...
                Config.fileWrite.log("File size check failed: File count mismatch", Loader.LOG_FILE_NAME);
                return false;
            }
            Config.fileWrite.log("File size check passed", Loader.LOG_FILE_NAME);
            return true;
        } catch (Exception e) {
            Config.exceptionHandler.handleException(e);
            Config.fileWrite.log("File size check error: " + e.getMessage(), Loader.LOG_FILE_NAME);
            return false;
        }
    }
    ...
    private boolean setupStatusCheck() throws Exception {
        try {
            ...
            Config.fileWrite.log("Setup status check: " + (exists ? "Passed" : "Failed"), Loader.LOG_FILE_NAME);
            return exists;
        } catch (Exception e) {
            Config.exceptionHandler.handleException(e);
            Config.fileWrite.log("Setup status check error: " + e.getMessage(), Loader.LOG_FILE_NAME);
            return false;
        }
    }
}
...
class GuestShell {
    public void execute() throws Exception {
        try {
            Config.fileWrite.log("Starting GuestShell", Loader.LOG_FILE_NAME);
            String input;
            do {
                input = Config.console.readLine("> ");
                String[] commandArray = Config.io.splitStringToArray(input);
                Config.fileWrite.log("GuestShell command: " + input, Loader.LOG_FILE_NAME);
                ...
                switch (commandArray[0].toLowerCase()) {
                    ...
                    case "clear":
                        ...
                        Config.fileWrite.log("GuestShell: Cleared screen", Loader.LOG_FILE_NAME);
                        break;
                    case "login":
                        new SycoraxKernel().startSycoraxKernel();
                        Config.build.viewBuildInfo(false);
                        Config.io.println("Logout Successful");
                        Config.fileWrite.log("GuestShell: Login completed, logged out", Loader.LOG_FILE_NAME);
                        break;
                    default:
                        Config.io.printError(commandArray[0] + " Command Not Found.");
                        Config.fileWrite.log("GuestShell error: Command not found - " + commandArray[0], Loader.LOG_FILE_NAME);
                        break;
                }
            } while (!input.equalsIgnoreCase("exit"));
            Config.fileWrite.log("GuestShell terminated", Loader.LOG_FILE_NAME);
        } catch (Exception e) {
            Config.exceptionHandler.handleException(e);
            Config.fileWrite.log("GuestShell error: " + e.getMessage(), Loader.LOG_FILE_NAME);
...
class Setup {
    ...
    boolean setupCataphract() throws Exception {
        try {
            Config.fileWrite.log("Starting Cataphract setup", Loader.LOG_FILE_NAME);
            ...
            Config.fileWrite.log("Displayed setup introduction", Loader.LOG_FILE_NAME);

            if (!showAndAcceptEULA()) {
                Config.fileWrite.log("Setup failed: EULA not accepted", Loader.LOG_FILE_NAME);
                return false;
            }
            if (!createSystemDirectories()) {
                Config.fileWrite.log("Setup failed: Directory creation failed", Loader.LOG_FILE_NAME);
                return false;
            }
            if (!initializeDatabase()) {
                Config.fileWrite.log("Setup failed: Database initialization failed", Loader.LOG_FILE_NAME);
                return false;
            }
            if (!initializeDefaultPolicies()) {
                Config.fileWrite.log("Setup failed: Policy initialization failed", Loader.LOG_FILE_NAME);
                return false;
            }
            if (!createAdministratorAccount()) {
                Config.fileWrite.log("Setup failed: Admin account creation failed", Loader.LOG_FILE_NAME);
                return false;
            }

            displaySetupProgress();
            Config.io.confirmReturnToContinue("Setup complete! ", ".\nSetup> ");
            Config.fileWrite.log("Setup completed successfully", Loader.LOG_FILE_NAME);
        } catch (Exception e) {
            Config.exceptionHandler.handleException(e);
            Config.fileWrite.log("Setup error: " + e.getMessage(), Loader.LOG_FILE_NAME);
        }
    ...
    private void displaySetupProgress() throws Exception {
        try {
            ...
            Config.io.println("[ ----------------------------- ]\n");
            Config.fileWrite.log("Displayed setup progress", Loader.LOG_FILE_NAME);
        } catch (Exception e) {
            Config.exceptionHandler.handleException(e);
            Config.fileWrite.log("Setup progress display error: " + e.getMessage(), Loader.LOG_FILE_NAME);
    ...
    private boolean showAndAcceptEULA() throws Exception {
        try {
            ...
            Config.fileWrite.log("EULA acceptance: " + (accepted ? "Accepted" : "Not accepted"), Loader.LOG_FILE_NAME);
            if (accepted) {
                Config.fileRead.readHelpFile("LICENSE");
                prereqInfoStatus = true;
            }
        } catch (Exception e) {
            Config.exceptionHandler.handleException(e);
            Config.fileWrite.log("EULA acceptance error: " + e.getMessage(), Loader.LOG_FILE_NAME);
        }
        ...
        try {
            ...
            for (String dir : directoryNames) {
                ...
                if (!directory.exists() && !directory.mkdirs()) {
                    ...
                    Config.fileWrite.log("Directory creation failed: " + dir, Loader.LOG_FILE_NAME);
                    initDirs = false;
                    break;
                }
            }
            Config.fileWrite.log("System directories created successfully", Loader.LOG_FILE_NAME);
            initDirs = true;
        } catch (Exception e) {
            Config.exceptionHandler.handleException(e);
            Config.fileWrite.log("Directory creation error: " + e.getMessage(), Loader.LOG_FILE_NAME);
        }
        return initDirs;
    }
    ...
    private boolean initializeDatabase() throws Exception {
        try {
            ...
            Config.fileWrite.log("Database initialized " + (initDB ? "successfully" : "unsuccessfully"), Loader.LOG_FILE_NAME);
        } catch (Exception e) {
            Config.exceptionHandler.handleException(e);
            Config.fileWrite.log("Database initialization error: " + e.getMessage(), Loader.LOG_FILE_NAME);
        }
        return initDB;
    }
    ...
    private boolean initializeDefaultPolicies() throws Exception
    {
        ...
        try {
            ...
            if (!policiesInitialized) {
                ...
                Config.fileWrite.log("Policy initialization failed: Policy file not created", Loader.LOG_FILE_NAME);
            }
            Config.fileWrite.log("Default policies initialized successfully", Loader.LOG_FILE_NAME);
            initPolicies = true;
        } catch (Exception e) {
            Config.exceptionHandler.handleException(e);
            Config.fileWrite.log("Policy initialization error: " + e.getMessage(), Loader.LOG_FILE_NAME);
        }
        return initPolicies;
    }
    ...
    private boolean createAdministratorAccount() throws Exception {
        try {
            ...
            Config.fileWrite.log("Administrator account created successfully", Loader.LOG_FILE_NAME);
            initAdminAccount = true;
        } catch (Exception e) {
            Config.exceptionHandler.handleException(e);
            Config.fileWrite.log("Admin account creation error: " + e.getMessage(), Loader.LOG_FILE_NAME);
        }
